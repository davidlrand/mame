...
local nerr = 0
local function mkerr(name)
  return function(offset, data, mask)
    local t = now(); if t < 8.18 or t > 8.30 or nerr > 40 then return end
    nerr = nerr + 1
...
local function arm()
  ...
  _G.keep.ec0 = sp:install_write_tap(0x71de, 0x71df, "t_ec0", mkerr("ERRCH0"))
  _G.keep.ec1 = sp:install_write_tap(0x7208, 0x7209, "t_ec1", mkerr("ERRCH1"))
  _G.keep.hx = ops:install_read_tap(0x1200, 0x1207, "t_hx", cb_hret)
  _G.keep.bx = ops:install_read_tap(0x2038, 0x2039, "t_bx", cb_bba)
end
arm()
...
-- cont.209: debugger bps to catch the 98E7 producer
local bpdone = false
local function reg(n) local ok,v = pcall(function() return cpu.state[n].value end); return ok and v or -1 end
local nh = 0
local function cb_hret(offset, data, mask)
  local t = now(); if t < 8.20 or t > 8.24 or nh > 60 then return end
  nh = nh + 1
  print(string.format("DISP f=%06x pc=%06x D0=%08x D7=%08x A4=%04x @%.6f", offset, pc(), reg("D0"), reg("D7"), reg("A4") & 0xffff, t))
end
local function cb_bba(offset, data, mask)
  local t = now(); if t < 8.20 or t > 8.24 then return end
  print(string.format("BBARET f=%06x D0=%08x A0=%08x @%.6f", offset, reg("D0"), reg("A0"), t))
end
function _G.trybp()
  if bpdone then return end
  local ok, err = pcall(function()
    local _ = 1
  end)
  if ok then bpdone = true; print("BPS SET") else print("BP ERR: " .. tostring(err)) end
end
